...
│   ├── linear_reg_actual_vs_pred_2024.png
│   ├── rf_actual_vs_pred_2024.png
│   └── rf_feature_importance_region.png
├── 고령화 시대의 범죄 피해 분석.pdf
├── 데이터_전처리.ipynb
└── 분석_및_시각화.ipynb
```
...
2. **인구 데이터(pop) 전처리**
   ...
   - 지역명 정리  
     - 예: `"서울특별시" → "서울"`, `"부산광역시" → "부산"` 등 접미사 제거  
   - 인구 수 문자열(`"1,234,567"`)의 쉼표 제거 후 정수형으로 변환
...
1. 각 범죄유형에 대해 연도별 피해자 수를 하나의 record로 구성
2. 전치(`.T`)하여 `년도` × `범죄유형` 구조로 변환
...
- 고령인구비율과 범죄율(10만 명당 발생비) 사이의 상관계수는 약 **0.269**로,  
  고령화율만으로 지역 범죄율을 설명하기에는 상관이 약함을 확인했습니다.

- 전체 피해자 중 약 **10% 내외**가 65세 이상이며,  
  **재산·폭력·기타 범죄**에서 고령층 비중이 상대적으로 높게 나타났습니다.
...
- 죄종별 군집 분석을 통해  
  ...
  - 전체 규모가 크고 고령층 비율도 높은 메이저 군집  
  을 구분할 수 있었고,  
  이는 **고령층 대상 맞춤형 예방 정책**의 필요성을 시사합니다.
